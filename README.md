...
This is a project that deploys an nginx webserver as the end product using terraform commnands running in a jenkins pipeline. The project is in two phases initially, i had to provision the jenkins server using terraform (IAC) and within the script that produces the jenkins software i inserted some scripts to install terraform and aws cli

In the secoond phase once jenkins is configured now using jenkins pipeline,to run another terraform commands that will first create an aws eks cluster and change directory to the manifest folder to deploy the nginx to aws cloud managed kubernetes cluster(aws-eks).
...
**📌 Step 12: Cleaup Respources deployed**
